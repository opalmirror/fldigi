    ...
    if(file_cmd)
      ...
      if("${file_ov}" MATCHES "executable")
        ...
        if("${file_ov}" MATCHES "text")
          message("but text, so *not* a binary executable!")
        else()
          set(${result_var} 1 PARENT_SCOPE)
          return()
        endif()
...
function(gp_resolve_item context item exepath dirs resolved_item_var)
  ...
  if(NOT resolved)
    if(item MATCHES "@executable_path")
      ...
      if(EXISTS "${ri}")
        message("info: embedded item exists (${ri})")
        set(resolved 1)
        set(resolved_item "${ri}")
      else()
        message("warning: embedded item does not exist '${ri}'")
      endif()
    endif()
  endif()
  ...
  if(NOT resolved)
    if(item MATCHES "@rpath")
      ...
      if(ri)
        message("info: 'find_file' in exepath/dirs (${ri})")
        set(resolved 1)
        set(resolved_item "${ri}")
        set(ri "ri-NOTFOUND")
      endif()
  ...
  if(NOT resolved)
    if(item MATCHES "[^/]+\\.framework/")
      ...
      if(fw)
        message("info: 'find_file' found framework (${fw})")
        set(resolved 1)
  ...
  if(NOT resolved)
    ...
    if(ri)
      message("info: 'find_program' in exepath/dirs (${ri})")
      set(resolved 1)
      set(resolved_item "${ri}")
      set(ri "ri-NOTFOUND")
    endif()
...
function(gp_resolved_file_type original_file file exepath dirs type_var)
  ...
  if(NOT IS_ABSOLUTE "${original_file}")
    message("warning: gp_resolved_file_type expects absolute full path for first arg original_file")
  endif()
  ...
  message("gp_resolved_file_type: '${file}' '${resolved_file}'")
  message("                type: '${type}'")

  if(NOT is_embedded)
    if(NOT IS_ABSOLUTE "${resolved_file}")
      if(lower MATCHES "^msvc[^/]+dll" AND is_system)
        message("info: non-absolute msvc file '${file}' returning type '${type}'")
      else()
        message("warning: gp_resolved_file_type non-absolute file '${file}' returning type '${type}' -- possibly incorrect")
      endif()
    endif()
  endif()
...
function(gp_file_type original_file file type_var)
  if(NOT IS_ABSOLUTE "${original_file}")
    message("warning: gp_file_type expects absolute full path for first arg original_file")
  endif()
...
function(get_prerequisites target prerequisites_var exclude_system recurse exepath dirs)
  ...
  message("Tool found: ${gp_tool}")

  if(NOT gp_cmd)
    message("warning: could not find '${gp_tool}' - cannot analyze prerequisites...")
    return()
  endif()
  ...
  if(NOT gp_tool_known)
    message("warning: gp_tool='${gp_tool}' is an unknown tool...")
    message("CMake function get_prerequisites needs more code to handle '${gp_tool}'")
    message("Valid gp_tool values are dumpbin, ldd, objdump and otool.")
    return()
  endif()
  ...
  if(verbose)
    message("<RawOutput cmd='${gp_cmd} ${gp_cmd_args} ${target}'>")
    message("gp_cmd_ov='${gp_cmd_ov}'")
    message("</RawOutput>")
  endif()
  ...
  if("${candidate}" MATCHES "${gp_regex}")
    ...
    if(verbose)
      message("ignoring non-matching line: '${candidate}'")
    endif()
  endif()
  endforeach()
...
function(list_prerequisites target)
  ...
  if(print_target)
    message("File '${target}' depends on:")
  endif()
...
function(list_prerequisites_by_glob glob_arg glob_exp)
  message("=============================================================================")
  message("List prerequisites of executables matching ${glob_arg} '${glob_exp}'")
  message("")
